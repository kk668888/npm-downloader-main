      ...
      <Teleport to="body">
        ...
        >
          <div
            ...
            class="fixed z-[100] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-soft-lg overflow-hidden"
            :style="dropdownStyle"
          >
            <div class="max-h-64 overflow-y-auto">
              <div
                ...
                :key="pkg.name"
                class="px-3 py-2 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                :class="{ 'bg-slate-100 dark:bg-slate-700': focusedIndex === index }"
                @mousedown="selectSuggestion(pkg)"
                @mouseenter="focusedIndex = index"
              >
                <div class="flex items-center justify-between">
                  <div class="flex-1 min-w-0">
                    <div class="font-medium text-sm truncate text-slate-900 dark:text-slate-100">
                      {{ pkg.name }}
                    </div>
                  ...
                  <div class="text-xs text-slate-400 dark:text-slate-500 ml-2">
                    {{ pkg.version }}
                  </div>
                </div>
              </div>
            </div>
            <div v-if="isSearching" class="px-3 py-2 text-sm text-slate-500 dark:text-slate-400 text-center">
              搜索中...
            </div>
          </div>
        </Transition>
...
const updateInputPosition = () => {
  const input = inputElement.value?.$el?.querySelector('input');
  if (input) {
